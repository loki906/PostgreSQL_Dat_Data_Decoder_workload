    ...
    def get_roi_cells_by_range(self, lower_bound, upper_bound):
        ...
        try:
            ...
            cursor.execute("""
                ...
                AND variable = 'X' AND is_element = true
            """, (self.ship_type, self.scale))
            x_data = cursor.fetchone()['data']
            if not x_data:
                cursor.close()
                return []
            ...
            cursor.execute("""
                SELECT data FROM cae_simulation_data 
                WHERE ship_type = %s AND scale = %s 
                AND variable = 'Y' AND is_element = true
            """, (self.ship_type, self.scale))
            y_data = cursor.fetchone()['data']
            
            cursor.execute("""
                SELECT data FROM cae_simulation_data 
                WHERE ship_type = %s AND scale = %s 
                AND variable = 'Z' AND is_element = true
            """, (self.ship_type, self.scale))
            z_data = cursor.fetchone()['data']
            
            # 过滤X/Y/Z都在范围内的索引（0基）
            valid_indices = []
            for idx in range(len(x_data)):
                if (lower_bound[0] <= x_data[idx] <= upper_bound[0]  # X轴范围
                    and lower_bound[1] <= y_data[idx] <= upper_bound[1]  # Y轴范围
                    and lower_bound[2] <= z_data[idx] <= upper_bound[2]):  # Z轴范围
                    valid_indices.append(idx)
            
            cursor.close()
            debug_print(f"ROI查询结果: {len(valid_indices)}个单元格", "时间聚合", "ROI查询")
